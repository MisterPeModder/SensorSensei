...
pub struct InfluxDBConfig {
    ...
    /// If not specified, defaults to "sensors_org"
    pub org: &'static str,
    /// Bucket name in InfluxDB where data will be written
    /// If not specified, defaults to "sensors"
...
fn get_runtime_config() -> Config {
    ...
    let influx_db = match (
        ...
        (Some(host), Some(token)) => Some(InfluxDBConfig {
            host,
            port: ENVIRONMENT_VARIABLES
                .influx_db_port
                .and_then(|p| p.parse().ok())
                .unwrap_or(8086),
            org: ENVIRONMENT_VARIABLES.influx_db_org.unwrap_or("sensors_org"),
            bucket: ENVIRONMENT_VARIABLES.influx_db_bucket.unwrap_or("sensors"),
            api_token: token,
        }),
        _ => None,
    };
