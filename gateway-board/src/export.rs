...
impl ValuesExporter for InfluxDbExporter {
    async fn export(
        ...
        let mut path_buf: heapless::String<100> = heapless::String::new();
        _ = write!(
            &mut path_buf,
            "api/v2/write?org={}&bucket={}&precision=s",
            self.cfg.org, self.cfg.bucket
        );
        // Tokens generated by InfluxDB are 88 characters long by default, so 94 with the "Token " prefix
        let mut authorization_buf: heapless::String<94> = heapless::String::new();
        _ = write!(&mut authorization_buf, "Token {}", self.cfg.api_token);

        let mut req = client
            .request(HttpMethod::Post, self.cfg.host, self.cfg.port, path_buf)
            .await?;
        req.header("Content-Type", "text/plain").await?;
        req.header("Authorization", &authorization_buf).await?;
