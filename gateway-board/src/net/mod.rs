...
use crate::export::{SensorCommunityExporter, ValuesExporter};
...
pub async fn http_client_demo(stack: Stack<'_>) -> Result<(), HttpClientError> {
    let mut client = http::HttpClient::new(stack);
    let values = [SensorValue::Temperature(24f32)];
    let ex = SensorCommunityExporter;

    ex.export(&mut client, &values).await
}
